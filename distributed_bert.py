...
class BertSelfOutput(nn.Module):
    ...
    # def break_fc(self, breakdown):
    #     if len(breakdown) != self.num_attention_heads: # Check if the total number of neurons on all devices matches the hidden_size
    #         # raise ValueError(f'Bad breakdown: {breakdown}. Sum of numbers should be equal to hidden_size({self.dense.weight.shape[1]})')
    #         raise ValueError(f'Len of breakdown {breakdown} should be {self.num_attention_heads}!')
        
    #     self.breakdown = True
    #     self.breakdown_mask = torch.ones_like(self.dense.weight)

    #     insize = self.dense.weight.shape[0]//self.num_attention_heads
    #     outsize = self.dense.weight.shape[1]//self.num_attention_heads

    #     for i in range(len(breakdown)):
    #         for j in range(len(breakdown)):
    #             if breakdown[i] != breakdown[j]:
    #                 self.breakdown_mask[insize*i: insize*(i+1), outsize*j:outsize*(j+1)] = 0

    #     if self.custom_config['verbose']:
    #         print(f'Breakdown Mask Sum:{(self.breakdown_mask>0.001).sum()}')
    #     # 1/0
...
class MyBertLayer(BertLayer):
    ...
    def set_custom_config(self, custom_config):
        ...
        self.intermediate.set_custom_config(custom_config)
        self.output.set_custom_config(custom_config)
    ...
    def break_fcs(self, breakdown):
        num_devices = len(self.custom_config['active_probs'])
        self.intermediate.fc_masks = {}
        self.output.fc_masks = {}
        self.attention.output.fc_masks = {}
        
       
        i_in = self.intermediate.dense.weight.shape[1]//len(breakdown)
        i_out = self.intermediate.dense.weight.shape[0]//len(breakdown)

        o_in = self.output.dense.weight.shape[1] //len(breakdown)
        o_out = self.output.dense.weight.shape[0] //len(breakdown)

        ao_in = self.attention.output.dense.weight.shape[1]//len(breakdown)
        ao_out = self.attention.output.dense.weight.shape[0]//len(breakdown)

        for i in range(num_devices):
            self.intermediate.fc_masks[i] = torch.zeros_like(self.intermediate.dense.weight)
            self.output.fc_masks[i] = torch.zeros_like(self.output.dense.weight)
            self.attention.output.fc_masks[i] = torch.zeros_like(self.attention.output.dense.weight)

        for i in range(len(breakdown)):
            self.attention.output.fc_masks[breakdown[i].item()][:, ao_in*i: ao_in*(i+1)] += 1
            self.output.fc_masks[breakdown[i].item()][:, o_in*i: o_in*(i+1)] += 1
            self.intermediate.fc_masks[breakdown[i].item()][i_out*i: i_out*(i+1), :] += 1
        
        self.intermediate.breakdown = True
        self.output.breakdown = True
        self.attention.output.breakdown = True
        # for j in range(num_devices):
        #     print(j)
        #     print(self.attention.output.fc_masks[j].sum())
        #     print(self.output.fc_masks[j].sum())
        #     print(self.intermediate.fc_masks[j].sum())
...
class MyBertEncoder(BertEncoder):
    ...
    def break_fcs(self):
        for i, layer_module in enumerate(self.layer):
            if self.custom_config['verbose']:
                print(self.custom_config['distribution_matrix'][i], layer_module.attention.self.attention_head_size)
            layer_module.break_fcs(self.custom_config['distribution_matrix'][i])
    ...
    def set_custom_config(self, custom_config):
        ...
        # if 'break_fcs' in custom_config and custom_config['break_fcs'] and not self.fcs_broken:
        #     self.fcs_broken = True
        #     self.break_fcs()
        #     print("Breaking Fully-Connected Layers to multiple FCs on Separate Devices!")
...
class MyBertModel(BertModel):
    ...
    def forward(
        ...
        # Remove the unavailable embeddings
        if self.custom_config['embedding_churn_simulation'] == True:
        ...
        if self.custom_config['embedding_churn_simulation'] == True:
            if self.custom_config['verbose']:
                print(f'Embeddings output sum: {embedding_output.sum()}')
            # Reactivate embeddings
...
class MyBert(BertForSequenceClassification):
    ...
    def set_custom_config(self, custom_config):
        self.custom_config = custom_config
        # self.bert.encoder.fcs_broken = False
        self.bert.set_custom_config(custom_config)

        if 'break_fcs' in custom_config and custom_config['break_fcs']:
            print('Breaking fully-connected nets across devices! This line should not be printed more than once during the training process.')
            self.bert.encoder.break_fcs()
...
###################################################################
def get_stoch_bert_for_classification(base="bert-base-uncased", num_labels = 5, output_attentions = False, output_hidden_states = False, custom_config = {}):
